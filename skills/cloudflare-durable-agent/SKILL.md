...
- **Custom domain:** `wrangler deploy --domain agent.example.com`. If the zone has a catch-all Worker route (`*/*`, common with Cloudflare for SaaS), it still wins over the custom domain; add a more specific route too: `--route "agent.example.com/*" --x-route-zones --zone example.com`. Deploying with `--domain` turns `workers.dev` off unless `"workers_dev": true` is set.
